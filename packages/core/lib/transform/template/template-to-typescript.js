    ...
    return mapTemplateContents(originalTemplate, { embeddingSyntax }, (ast, mapper) => {
                ...
                if (!node.selfClosing) {
                    ...
                    if (blocks.type === 'named') {
                        for (const child of blocks.children) {
                            ...
                            emit.forNode(child, () => emitBlockContents(name, nameStart, child.blockParams, blockParamsStart, child.children));
                        }
                    }
                    else {
        ...
        function determineBlockChildren(node) {
            ...
            if (named === 0) {
                ...
                for (let child of node.children) {
                    if (!isNamedBlock(child)) {
                        emit.forNode(child, () => assert(isAllowedAmongNamedBlocks(child), 'Named blocks may not be mixed with other content'));
                    }
                }
        ...
        function emitSpecialFormStatement(formInfo, node) {
            ...
            switch (formInfo.form) {
                ...
                case 'if-not':
                    ...
                    break;
                case 'bind-invokable':
                    record.error(`The {{${formInfo.name}}} helper can't be used directly in block form under Glint. ` +
        ...
        function emitBlock(name, node) {
            let paramsStart = template.lastIndexOf('|', template.lastIndexOf('|', rangeForNode(node).start) - 1);
        ...
        function emitLiteral(node) {
            emit.forNode(node, () => emit.text(node.value === undefined ? 'undefined' : JSON.stringify(node.value)));
        }
