...
export function bindLanguageServerPool({ connection, pool, openDocuments }) {
    ...
    connection.onCodeAction(({ textDocument, range, context }) => {
        return pool.withServerForURI(textDocument.uri, ({ server }) => {
            ...
            if (context.triggerKind === CodeActionTriggerKind.Invoked) {
                ...
                return server.getCodeActions(textDocument.uri, kind, range, diagnostics, FORMATTING_OPTIONS, PREFERENCES);
            }
            return [];
        });
    });
